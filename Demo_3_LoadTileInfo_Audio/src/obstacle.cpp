...
int Obstacle::CollisionSide(CIwFVec2 characterPos , CIwSVec2 characterBox, CIwFVec2 &target)
{
	...
	case 0://inside
		break;
	case 1://top
		target.y=limit_U-(target.y-limit_U)*0.1f;;
		break;
	case 2://right
		target.x=limit_R-(target.x-limit_R)*0.1f;
		break;
	case 3://bottom
		target.y=limit_D-(target.y-limit_D)*0.1f;
		break;
	case 4://left
		target.x=limit_L-(target.x+limit_L)*0.1f;
		break;
	}
