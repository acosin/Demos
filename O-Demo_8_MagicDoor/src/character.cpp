...
void Character::Load()
{
	...
	_anima_left=new Anima;
	//m_MOVERANGE=50.0f;
	m_HitRange=false;

	//_image=Iw2DCreateImageResource("character");
	_anima_right->Load("right",5,100);
	_anima_idle->Load("idle",2,300);
	_anima_left->Load("left",5,100);
...
void Character::Update(int deltaTime)
{
	...
	{
		_anima_left->SetPos(CIwSVec2(iwsfixed(m_ScreenCenter.x-_anima_idle->_frameSize.x/2.0f), iwsfixed(m_ScreenCenter.y-_anima_idle->_frameSize.y/2.0f-_anima_idle->_frameSize.y/4.0f)));
		_anima_left->DoAnima(deltaTime);
	}
}
