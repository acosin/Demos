...
void Character::Update(int deltaTime)
{
	...
	if(_CS==idle)
	{
		_anima_idle->SetPos(CIwSVec2(iwsfixed(m_ScreenCenter.x-_Size.x/2.0f), iwsfixed(m_ScreenCenter.y-_Size.y/2.0f-_Size.y/4.0f)));
		_anima_idle->DoAnima(deltaTime);
	}
	else if(_CS==right)
	{
		_anima_right->SetPos(CIwSVec2(iwsfixed(m_ScreenCenter.x-_Size.x/2.0f), iwsfixed(m_ScreenCenter.y-_Size.y/2.0f-_Size.y/4.0f)));
