...
void TileObstacle::InitialObstacle(CIwFVec2 pos,CIwSVec2 size)
{
	...
	_Size=size;
	bool display=false;
	m_Edges.push_back(CreateObstacle(CIwFVec2(m_Position.x,m_Position.y),CIwSVec2(_Size.x,OBSTACLESIZE),display));
	m_Edges.push_back(CreateObstacle(CIwFVec2(m_Position.x+(float)_Size.y-OBSTACLESIZE,m_Position.y),CIwSVec2(OBSTACLESIZE,_Size.y),display));
	m_Edges.push_back(CreateObstacle(CIwFVec2(m_Position.x,m_Position.y+(float)_Size.y-OBSTACLESIZE),CIwSVec2(_Size.x,OBSTACLESIZE),display));
	m_Edges.push_back(CreateObstacle(CIwFVec2(m_Position.x,m_Position.y),CIwSVec2(OBSTACLESIZE,_Size.y),display));
